...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    [self.textView selectWordAtPoint:[touch locationInView:self.textView] withBlock:^(NSString *word, CGRect bbox) {
        if(word) {
            ...
            self.wordHighlight = [[UIView alloc] initWithFrame:wordFrame];
            self.wordHighlight.backgroundColor = [UIColor colorWithRed:1.0 green:0.0 blue:0.0 alpha:0.5];
            [self.view addSubview:self.wordHighlight];
        }
    }];
...
- (IBAction)botonDado:(id)sender {
    if(!self.textView) {
        self.textView = [[CTHView alloc] initWithFrame:CGRectMake(40, 30, 270, 600)]; //con 280 de ancho falla
